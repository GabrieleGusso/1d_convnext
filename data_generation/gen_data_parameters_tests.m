function gen_data_parameters_tests(n_pulsars, freq_start, bandwidth, start_mjd, end_mjd, var_value, var_param)
	...
	%    var_value   value of the parameter that is being varied during the generation of the dataset
	...
	for i = 1:n_pulsars
		% if mod(i,100) == 0
		% 	text = "Percent done: " + num2str(i/n_pulsars*100) + "%";
		% 	disp(text)
		% end
		h0 = h_min+rand(1)*(h_max-h_min); % min+rand*(max-min)
