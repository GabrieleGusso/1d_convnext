...
% Script to generate the different test datasets in real O3 noise (10 test datasets for each parameter)
...
for ii = 1:10
	name_dir = ['THESIS_TEST_realnoise_f_70-80_h0_' num2str(ii*5) 'e25_P_19_ecc_0_asini_1_cosi_0'];
	...
	parfor time_idx = 0:time_tot_bands-1
		...
		for kpuls = start:n_pulsars
			...
			%% NOISE
			noise   = bsd_lego_gusso(DEVBSD,det,data_tobeused,[start_mjd end_mjd],[fstart fend],1);
			noise   = cut_bsd(noise, [start_mjd end_mjd]);
			...
			%% SIGNAL
			save_signal(kpuls, database_idx, train_val, start_mjd, end_mjd, noise, name_dir);
			if time_idx == 0
