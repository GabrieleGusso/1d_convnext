...
class Block(nn.Module):
    ...
    We use (2) as we find it slightly faster in yPyTorch
...
class ConvNeXt(nn.Module):
    ...
    def _init_weights(self, m):
        if isinstance(m, (nn.Conv1d, nn.Linear)):
            trunc_normal_(m.weight, mean=0.0, std=0.2) # nell'articolo indicano 0.2 ma nel code release 0.02! <--- INSERIRE NELLA TESI
...
class LayerNorm(nn.Module):
    ...
    shape (batch_size, length, channels) while channels_first corresponds to inputs
    with shape (batch_size, channels, length).
    """
